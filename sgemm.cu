...
__global__ void mySgemmV1Aligned(
    ...
    // one block load A[128, 8] & B[8, 128]
    // one thread load 4 float32 of A & 4 float32 of B
...
__global__ void mySgemmV2Aligned(
    ...
    for (int bk = 0; bk < (K + BK - 1) / BK; bk++) {
        ...
        for (int tk = 0; tk < BK; tk++) {
            FLOAT4(r_comp_a[0]) = FLOAT4(s_a[tk][ty * TM    ]);
            FLOAT4(r_comp_a[4]) = FLOAT4(s_a[tk][ty * TM + 4]);
            FLOAT4(r_comp_b[0]) = FLOAT4(s_b[tk][tx * TN    ]);
            FLOAT4(r_comp_b[4]) = FLOAT4(s_b[tk][tx * TN + 4]);
...
__global__ void mySgemmV4Aligned(
    float * __restrict__ a, float * __restrict__ b, float * __restrict__ c,
    const int M, const int N, const int K) {
